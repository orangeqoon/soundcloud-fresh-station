...
### ⚠️ 注意点 4: 音量制御（v1.7.6〜 SoundCloud 本体の音量モジュールを使用）
- SoundCloud は「全体音量」を内部に持ち、曲が変わるたびに新しいプレイヤーへ再適用する。そのため `audio.volume` を直接書き換えても次の曲で戻る。
- `setSoundCloudVolume()` は webpack 経由で取得した SoundCloud の音量モジュール（`setVolumeAndMuted` / `getVolume` / `getMuted` / `setMuted` を持つもの）を呼ぶ。本体スライダー・`V2::local::settings` の保存値とも同期する。
- SoundCloud は 0.1 刻みでしか保存しないため、再読込後に一度だけ `sc_fresh_station_volume` の細かい値へ戻す（`restoreVolumePrecisionOnce`）。
- 音量モジュールが見つからない場合のみ、`play` イベント／`HTMLMediaElement.prototype.play` フックで `audio.volume` に保存値を再適用するフォールバックが動く。

### ⚠️ 注意点 4b: Windows メディア操作パネル（Media Session）
- SoundCloud 本体が play / pause / nexttrack / previoustrack / seekforward / seekbackward と曲情報（複数サイズのジャケット付き）を自前で登録する。**これらを上書きしないこと**（以前は再生/一時停止を「トグルボタンのクリック」で上書きしており、状態が逆になることがあった）。
- 本拡張は本体が送らない分だけを補う：`setPositionState`（再生位置）、`seekto`（位置指定シーク）、`stop`。本体が曲ごとにハンドラーを登録し直すため `syncMediaSession()` が5秒毎に再登録する。
- 曲情報は本体が設定していない時だけ補い、前の曲のジャケットが残っている間は画像なしにする。
- 再生位置・シークは SoundCloud 再生管理モジュール（`playNext`/`playPrev`/`getCurrentSound`/`pauseCurrent` を持つもの）の現在サウンド（`currentTime()`/`duration()` はミリ秒、`seek(ms)`）を使う。
- タスクバーのサムネイル上のボタン（ThumbBar）は Windows ネイティブ API で、ウィンドウを持つ Edge 本体しか追加できないため、拡張機能からは不可能。
...
| OAuth トークン | **保存しない**。毎回 Cookie `oauth_token` → SoundCloud 認証モジュール → 通信から捕捉した値 → background 注入値 の順で最新を取得（`getAuthToken()`） | — | SoundCloud はトークンを定期更新するため、キャッシュを優先すると失効トークンを送り続けてアカウント接続に失敗する（v1.7.5 以前の不具合）。旧キー `sc_fresh_station_cache_oauth_token` は起動時に削除 |
...
- API v2 への通信は必ず `scApi(path, {method, body})` を使う。client_id と最新トークンを付与し、401 の場合はトークンを取り直して1回だけ再試行する。
- client_id は SoundCloud の設定モジュール（`get('client_id')`）から取得し、無ければ通信の捕捉・JSアセット走査にフォールバック。
- 内部モジュールは `findScModule(name, predicate)` で**形（関数名）で検索**している（webpack のモジュール番号はビルド毎に変わるため番号で参照しない）。
- 主なエンドポイント（SoundCloud 本体のルート定義で確認済み）: フォロー `POST me/followings/:id` / 解除 `DELETE`、リポスト `PUT me/track_reposts/:id` / 解除 `DELETE`、プレイリスト更新 `PUT playlists/:id`（`{playlist:{tracks:[ids]}}`、上限500曲）、一覧 `GET users/:id/playlists`。
...
## 9. Claude への相談・診てもらう用プロンプト例

Claudeに相談する際は、以下のテンプレートをコピー＆ペーストして使用してください：

```markdown
以下のSoundCloud用ブラウザ拡張機能「FreshDig for SoundCloud (v1.7.6)」のコードベースを診断・レビューしてください。
